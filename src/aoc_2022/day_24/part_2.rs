...
fn blizzard_basin(input_str: &str) -> usize {
    ...
    queue.push_back((0, start_pos, false, false));
    while let Some(state) = queue.pop_front() {
        ...
        let (time, cur_pos, mut is_end, mut is_start) = state;
        ...
        if (cur_pos.0, cur_pos.1) == start_pos && is_end {
            is_start = true;
        }
        ...
        for (delta_row, delta_col) in DIR.iter().chain([&(0,0)]) {
            ...
            queue.push_back((time + 1, new_pos, is_end, is_start));
        }
    }
    count
}
...
#<^v^^>#
######.#";
        assert_eq!(54, blizzard_basin(input_str));
    }

    #[test]
    fn test_blizzard_basin_from_file() {
        let input_str = include_str!("input.txt");
        assert_eq!(976, blizzard_basin(input_str));
    }
