...
fn not_enough_minerals(input_str: &str) -> i32 {
    let mut total = 0;
    for blueprint in input_str.lines() {
        let mut blueprint_iter = blueprint.split(':');

        let bl_idx = blueprint_iter.next().unwrap().split(' ').nth(1).unwrap().parse::<i32>().unwrap();
        
        let mut blueprint_iter = blueprint_iter.next().unwrap().split('.');
...
Blueprint 2: Each ore robot costs 2 ore. Each clay robot costs 3 ore. Each obsidian robot costs 3 ore and 8 clay. Each geode robot costs 3 ore and 12 obsidian.";
        assert_eq!(33, not_enough_minerals(input_str));
    }

    #[test]
    fn test_not_enough_minerals_from_file() {
        let input_str = include_str!("input.txt");
        assert_eq!(1264, not_enough_minerals(input_str));
    }
