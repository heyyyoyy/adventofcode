...
impl Monkey {
    ...
    fn analyze_item(&self, monkeys: &Vec<Monkey>, product: u64) {
...
fn monkey_in_the_middle(input_str: &str) -> u64 {
    ...
    let product =  monkeys.iter().map(|m| m.test_worry.div_by).product::<u64>();

    for _ in 0..10_000 {
        for monkey in monkeys.iter() {
            monkey.analyze_item(&monkeys, product);
        }
    }
...
";
        assert_eq!(2713310158, monkey_in_the_middle(input_str));
    }

    #[test]
    fn test_monkey_in_the_middle_from_file() {
        let input_str = include_str!("input.txt");
        assert_eq!(19573408701, monkey_in_the_middle(input_str));
    }
}
